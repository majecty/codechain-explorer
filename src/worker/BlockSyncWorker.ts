...
export class BlockSyncWorker {
    ...
    private indexingNewBlock = async (nextBlock: Block) => {
        ...
        // indexing parcels
        await Promise.all(
            _.map(nextBlock.parcels, async parcel => {
                const parcelDoc = await this.typeConverter.fromParcel(
                    parcel,
                    nextBlock.timestamp
                );
        ...
        _.each(nextBlock.parcels, parcel => {
            ...
            if (action instanceof ChangeShardState) {
                const jobs = _.map(
                    action.transactions,
                    async (transaction, i) => {
                        const transactionDoc = await this.typeConverter.fromTransaction(
                            transaction,
                            nextBlock.timestamp,
                            parcel,
                            i
                        );
                        return this.elasticSearchAgent.indexTransaction(
                            transactionDoc
                        );
                    }
                );
        ...
        if (parcelCount > 0) {
            ...
            const paymentParcelCount = _.filter(
                nextBlock.parcels,
                p => p.unsigned.action instanceof Payment
            ).length;
            await this.elasticSearchAgent.increaseLogCount(
                dateString,
            ...
            const serRegularKeyParcelCount = _.filter(
                nextBlock.parcels,
                p => p.unsigned.action instanceof SetRegularKey
            ).length;
            await this.elasticSearchAgent.increaseLogCount(
                dateString,
            ...
            const changeShardStateParcelCount = _.filter(
                nextBlock.parcels,
                p => p.unsigned.action instanceof ChangeShardState
            ).length;
            await this.elasticSearchAgent.increaseLogCount(
                dateString,
        ...
        const txs = _.chain(nextBlock.parcels)
            .filter(
        ...
        if (txCount > 0) {
            ...
            const assetMintTxCount = _.filter(
                txs,
                tx => tx instanceof AssetMintTransaction
            ).length;
            await this.elasticSearchAgent.increaseLogCount(
                dateString,
            ...
            const assetTransferTxCount = _.filter(
                txs,
                tx => tx instanceof AssetTransferTransaction
            ).length;
            await this.elasticSearchAgent.increaseLogCount(
                dateString,
        ...
        const sumOfParcelFee = _.reduce(
            nextBlock.parcels,
            (memo, parcel) =>
                new BigNumber(parcel.unsigned.fee.value.toString(10)).plus(
                    memo
                ),
            new BigNumber(0)
        ).toString(10);
        const miningReward = new BigNumber(sumOfParcelFee)
            .plus(
                new BigNumber(
        ...
        const paymentParcels = _.filter(
            nextBlock.parcels,
            parcel => parcel.unsigned.action instanceof Payment
        );
        const succeedPaymentParcelJob = _.map(
            paymentParcels,
            async (parcel: SignedParcel) => {
        ...
        for (const parcel of succeedPaymentParcels) {
            ...
            await this.elasticSearchAgent.increaseBalance(
                ...
                action.amount.value.toString(10)
            );
            await this.elasticSearchAgent.decreaseBalance(
                parcel.getSignerAddress().value,
                action.amount.value.toString(10)
            );
        }
    };
